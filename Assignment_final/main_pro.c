...
typedef int Status;
char largetmp[100000000];
char largestr[100000000];
char large_sign_word[100000000];
char large_name[100000000];
        ...
        while (small_flag || big_flag || !big_complete)
        {
            ...
            do
            {
                ...
                if (get_name)
                {
                    ...
                    for (; *p != '\0' && *p != '('; p++)
                    {
                        large_name[p - largestr] = *p;
                    }
                    large_name[p - largestr] = '\0';
                    if (qstrcmp(large_name, "main") == 0)
                    {
                        is_main = TRUE;
                    }
                    else
                    {
                        code->functions[code->functions_size].name = (char *)malloc(sizeof(char) * (strlen(large_name) + 1));
                        strcpy(code->functions[code->functions_size].name, large_name);
                    }
                    char c = '(';
                    fseek(fp, -strlen(largestr) + strlen(large_name), SEEK_CUR);
                    do
                    {
                        c = fgetc(fp);
                    } while (c != '{');
                ...
                if (is_main)
                {
                    ...
                    code->main_information_flow = (char *)malloc(sizeof(char) * (strlen(largetmp) + 1));
                    strcpy(code->main_information_flow, largetmp);
                    // puts(largetmp);
                }
                else
                {
                    ...
                    for (; *p != '\0';)
                    {
                        if (*p == '(')
                        {
                            ...
                            if (func_flag)
                            {
                                if (is_keepwords(&large_sign_word[0]))
                                {
                                    cur = strcat_cur(cur, largetmp, large_sign_word);
                                }
                                else
                                {
                                    largetmp[cur++] = 'F';
                        ...
                        {
                            if (is_keepwords(large_sign_word))
                            {
                                cur = strcat_cur(cur, largetmp, large_sign_word);
                            }
                            func_flag = FALSE;
                    ...
                    code->functions[code->functions_size].key_information_flow = (char *)malloc(sizeof(char) * (strlen(largetmp) + 1));
                    strcpy(code->functions[code->functions_size].key_information_flow, largetmp);
...
Status generate_program_key_information_flow(program *P)
{
        ...
        if (*p == '(')
        {
            ...
            if (func_flag)
            {
                ...
                else
                {
                    ...
                    {
                        if (qstrcmp(large_sign_word, P->functions[i].name) == 0)
                        {
    ...
    {
        strcat(largetmp, P->called_functions[i]->key_information_flow);
    }
    // puts(largetmp);
    P->key_information_flow = (char *)malloc(sizeof(char) * (strlen(largetmp) + 1));
    strcpy(P->key_information_flow, largetmp);
    return OK;
}
